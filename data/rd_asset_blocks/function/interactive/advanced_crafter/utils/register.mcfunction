...
# 明るさの定義
$execute store result storage rockietools:custom_crafter data."$(UUID)".current_brightness int 1 run function rd_asset_blocks:interactive/advanced_crafter/utils/get_brightness
...
# アイテムの定義
execute if block ~ ~ ~ barrel[open=true,facing=up] run function rd_asset_blocks:interactive/advanced_crafter/utils/counting_item with entity @s {}