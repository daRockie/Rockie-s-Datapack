...
$execute if entity @n[distance=1..,tag=!RD.shortBow,nbt={UUID:$(UUID)},type=#zombies,sort=furthest] unless entity @n[distance=0.0001..,type=#zombies,predicate=asset_mobs:in_hostile_to_each_other,tag=RD.archer_zombie,nbt={UUID:$(UUID)}] if entity @n[type=#zombies,distance=..2] run return fail
