$execute at @s run teleport @s ~ ~ ~ facing entity @n[tag=$(mob).target]
...
$execute at @s store result score @s xDis run data get entity @n[tag=$(mob).target] Pos[0] 500
$execute at @s store result score @s zDis run data get entity @n[tag=$(mob).target] Pos[2] 500
