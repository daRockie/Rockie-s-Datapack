...
execute if entity @s[type=drowned,tag=RD.squid_drowned] at @s run function custom_ai:custom_mobs/squid_drowned
...
execute rotated ~ 0 unless block ^ ^ ^1 #custom_ai:no_collision run scoreboard players reset @s[tag=!RD.no_chase] RD.ai_timer
execute rotated ~ 0 positioned ~ ~1 ~ unless block ^ ^ ^1 #custom_ai:no_collision run scoreboard players reset @s[tag=!RD.no_chase] RD.ai_timer

execute if entity @e[distance=2..4.5,tag=zombies.target] if score @s[tag=!RD.no_chase] RD.ai_timer matches 60.. run function custom_ai:advanced_ai/zombie/chase_jump
