...
execute if entity @s[tag=RD.zombie.fishing] run scoreboard players add @s RD.ai_timer 1
execute if score @s[predicate=rd_asset_mobs:in_hostile] RD.ai_timer matches 60.. run function custom_ai:advanced_ai/zombie/item/fishing_rod/bobber

execute if entity @s[tag=RD.zombie.fishing] run scoreboard players add @s RD.ai_timer_2 1
