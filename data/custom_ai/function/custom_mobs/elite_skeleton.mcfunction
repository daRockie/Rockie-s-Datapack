# AIタイマー加算
execute at @s if entity @e[tag=skeleton.target,distance=0..6] run scoreboard players add @s ai_timer 1
...
# ステップ（モブ相手）
execute if entity @s[nbt={OnGround:1b}] if entity @e[type=!wolf,type=!player,tag=skeleton.target,distance=0..16,sort=nearest,limit=1] if score @s ai_timer matches 60.. run function custom_ai:movements/step_away

# 追いかける
execute if entity @s[nbt={OnGround:1b}] unless items entity @s weapon.mainhand bow if entity @e[type=!wolf,tag=skeleton.target,distance=4..5] if score @s ai_timer matches 0..40 run function custom_ai:movements/chase_jump
