...
# 自分に攻撃してきたモブに敵対
execute on attacker run tag @s add skeleton_t
...
# ステップ（モブ相手）
execute if entity @s[nbt={OnGround:1b}] if entity @e[type=!wolf,type=!player,tag=skeleton_t,distance=0..16,sort=nearest,limit=1] if score @s ai_timer matches 60.. run function custom_ai:movements/step_away

# 追いかける
execute if entity @s[nbt={OnGround:1b}] unless items entity @s weapon.mainhand bow if entity @e[type=!wolf,tag=skeleton_t,distance=4..5] if score @s ai_timer matches 0..40 run function custom_ai:movements/chase_jump
