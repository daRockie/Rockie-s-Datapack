...
# マクロを再実行
execute if data storage rockietools:custom_recipe temp_crafter.meta.crafter.allowed_slot[-1] run function custom_items:custom_block/advanced_crafter/utils/storage/recipes/fill/ with storage rockietools:custom_recipe temp_crafter.meta.crafter.allowed_slot[-1]
...
# レシピ取得、アイテムデータを返却
function custom_items:custom_block/advanced_crafter/utils/storage/recipes/fill/final with storage rockietools:custom_recipe temp_crafter.meta